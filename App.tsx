...
const App = () => {
  ...
  const getWorkoutToday = async () => {
    try {
      const workouts = await HealthKit.queryWorkouts({
        ...
        endDate: +new Date(),
        key: HealthKitWorkoutType.Baseball,
      });

      // HealthKit.writeWorkout({
      //   startDate: startOfToday,
      //   endDate: +new Date(),
      //   key: HealthKitWorkoutType.Baseball,
      //   totalDistance: 100,
      ...
      HealthKit.deleteRecord({
        key: HealthKitDataType.Workout,
        startDate: new Date('2022-07-24T17:00:00Z'),
        endDate: new Date('2022-07-25T10:17:22Z'),
        uuid: 'C2EADE32-7BBE-4696-9B66-EB7CEBF3CEA9',
      });
  ...
  const getHeightAndWeight = async () => {
    try {
      ...
      const h_eight = await FitnessTracker.getLatestHeight();

      const w_eight = await FitnessTracker.getLatestWeight();
      ...
      console.log('[getHeightAndWeight] error ', error);
    }
  };
